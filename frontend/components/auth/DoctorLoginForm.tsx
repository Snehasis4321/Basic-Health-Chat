...
interface DoctorLoginFormProps {
  onSuccess?: (token: string, doctor: { id: string; email: string }) => void;
  onError?: (error: string) => void;
}

export default function DoctorLoginForm({ onSuccess, onError }: DoctorLoginFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/auth/doctor/login`, {
        method: 'POST',
        headers: {
      ...
      // Success
      onSuccess?.(data.token, data.doctor);
    } catch (err) {
